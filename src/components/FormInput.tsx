...
export default function FormInput(props: InputProps){
        {
            if(props.type !== 'textarea'){
                return(
                    <div className={`form__group flex flex-col bg-white mt-2 px-2 py-2 rounded-sm ${props.formGroup}`}>
                        <label aria-readonly className={`text-gray-400 text-sm px-2 
                        ${props.hiddenLabel ? 'hidden' : ''}`} htmlFor={props.name}>{props.title}</label>
                        <input 
                            id={props.name}
                            type={props.type} 
                            name={props.name}
